package com.android.calendar.datetimepicker.picker;
...
import com.android.calendar.datetimepicker.adapter.ArrayWheelAdapter;
import com.android.calendar.datetimepicker.listeners.OnItemPickListener;
import com.android.calendar.datetimepicker.util.DateUtils;
...
public class DateTimePicker extends WheelPicker {
    ...
    public void setSelectedItem(int year, int month, int day, int hour, int minute) {
        ...
        Log.d(TAG, "change months and days while set selected");
        ...
        if (timeMode != NONE) {
            ...
            if (hours.size() == 0) {
                Log.d(TAG, "init hours before make view");
                initHourData();
            }
    ...
    public void setSelectedItem(int yearOrMonth, int monthOrDay, int hour, int minute) {
        ...
        if (dateMode == MONTH_DAY) {
            ...
            Log.d(TAG, "change months while set selected");
    ...
    protected View makeCenterView() {
        ...
        if ((dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) && years.size() == 0) {
            Log.d(TAG, "init years before make view");
            initYearData();
        }
        if (dateMode != NONE && months.size() == 0) {
            Log.d(TAG, "init months before make view");
        ...
        if ((dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) && days.size() == 0) {
            Log.d(TAG, "init days before make view");
            int selectedYear;
            if (dateMode == YEAR_MONTH_DAY) {
        ...
        if (timeMode != NONE && minutes.size() == 0) {
            Log.d(TAG, "init minutes before make view");
            changeMinuteData(DateUtils.trimZero(selectedHour));
        }
